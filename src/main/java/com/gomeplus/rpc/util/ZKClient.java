...
import com.gomeplus.rpc.router.LoadBalanceRouter;
...
public class ZKClient {
    ...
    private static ConcurrentSkipListSet<Boolean> CONTAINS_SET2 = new ConcurrentSkipListSet<>();
    ...
    public static void getChildrenPath(final String rootPath) {
        try {
                ...
                public void process(WatchedEvent watchedEvent) {
                    if (watchedEvent.getType() == Watcher.Event.EventType.NodeChildrenChanged) {
                        ...
                        try {
                            ...
                            LoadBalanceRouter.IP_PORT_LIST.addAll(childrenPaths);
            ...
            if(LoadBalanceRouter.IP_PORT_LIST.isEmpty()){
                for (String ipPort : childrenPaths) {
                    if (!LoadBalanceRouter.CHANNEL_MAP.containsKey(ipPort)) {
                        ...
                        LoadBalanceRouter.CHANNEL_MAP.put(ipPort, channel);
                    }
    ...
    public static void getChildrenPath(final String rootPath, final ConcurrentSkipListSet<String> serverList) {
        try {
            ...
            List<String> childrenPaths = CLIENT.getChildren().usingWatcher(watcher).forPath(rootPath);
            serverList.addAll(childrenPaths);
            if(!CONTAINS_SET2.contains(true)){
                CONTAINS_SET2.add(true);
                CLIENT.getConnectionStateListenable().addListener(new ConnectionStateListener() {
                    @Override
                    public void stateChanged(CuratorFramework curatorFramework, ConnectionState connectionState) {
                        try {
    ...
    public void getRpcZKPath(final String ipPort, final String zkPath) {
    ...
    public void getZKPath(String ipPort, final String zkPath,ConcurrentSkipListSet<String> serverList){
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        try {
            initZkClient(ipPort,countDownLatch);
            countDownLatch.await();
